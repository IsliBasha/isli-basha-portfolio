...
- **Start button:** the flag on it is the boot splash's flag compressed onto the 16-grid — the same four VGA panes, dark-twin folds and black outline, drawn from the icon registry rather than the hand-mixed polygons it replaced, and held at 16px so the 16-grid lands on whole pixels at every display density. The 14px box the polygons rendered in resampled sixteen map columns into fourteen screen ones: three rows of the drawing disappeared and the outline came out three pixels wide at the top right where the map draws four. Going to 16px widens the button by 2px, which pushes the label and the task list right by the same and moves nothing else.
