...
async function fetchNeofetchOutput() {
  ...
  if (isMobile) return ['', ...statLines, ''].join('\n');

  const rowCount = Math.max(NEOFETCH_ART.length, statLines.length);
  const rows = [];
  for (let i = 0; i < rowCount; i++) {
    const artPart = (NEOFETCH_ART[i] ?? '').padEnd(ART_WIDTH);
    const statPart = statLines[i] ?? '';
    rows.push(`${artPart}${ART_GAP}${statPart}`);
  }
